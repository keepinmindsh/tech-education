...
- 뭔가 변경할 때마다 시스템이 의도대로 동작하지 않을
위험이 따른다.   
- 깨끗한 시스템은 클래스를 채계적으로 정리해 변경에 수반하는 위험을 낮춘다.    
...
OCP ( 개방 폐쇄의 원칙 ) - 확장에 개방적이지만, 수정에 폐쇄적이어야 하는 법칙   
DIP ( 의존관계역전 원칙 ) - 클래스가 상세한 구현이 아니라 추상화에 의존해야 한다는 것
